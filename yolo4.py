...
def infer(
    ...
    #print('blob_to_show.shape',blob_to_show.shape)  # (416, 416, 3)
    ...
    network = cv2.dnn.readNetFromDarknet(test_cfg_path,weights_path)
    ...
    # More colours [[111 204  28]] 
    #colours = np.random.randint(0, 255, size=(len(labels), 3), dtype='uint8')

    colours = np.array([[155, 211, 95]])
    ...
    #print('Objects Detection took {:.3f} seconds'.format(end - start))
    messages.append(f"Objects Detection took {round(end-start,3)} seconds for {img_name}")
    # """
    # End of:
    # Implementing Forward pass
    # """
    ...
    if len(results) > 0:
        ...
        for i in results.flatten():
            ...
            #print('Object {0}: {1} : {2}'.format(counter, labels[int(class_numbers[i])], round(confidences[i],2) ))
    ...
    messages.append(f"Total objects been detected: {len(bounding_boxes)}")
    t_box = len(bounding_boxes)
    #print('Number of objects left after non-maximum suppression:', counter - 1)
    messages.append(f"Number of objects left after non-maximum suppression: {counter-1}")
    box_nms = counter - 1
...
def grayscale(img_path, out_path):
